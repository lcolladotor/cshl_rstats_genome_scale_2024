...
Differential Gene Expression (DGE) analyses are common statistical analyses of gene expression data that aim to discover genes significantly altered in their expression levels between experimental groups, which can be given by a condition, treatment, experimental procedure/exposure, diagnostic, by biological origins (e.g. differences in sex, tissue, age), and even by different technical methodologies. These genes are known as Differentially Expressed Genes (DEGs) and can be either up- or down-regulated if their expression is greater or less in one group with respect to the other(s), respectively. Diverse methods exist to perform DGE and multiple downstream analyses can be applied on DEGs, but a series of non-skippable preliminary steps exists which are necessary to correctly perform previous to any statistical testing. 

Below a classic workflow for DGE is depicted. It takes as input the gene expression matrix with raw read counts for each gene (as rows) across all samples (as columns). Among the preliminary steps, there is an initial data processing step encompassing count normalization and filtering of lowly-expressed genes. Secondly, Exploratory Data Analysis (EDA) involves assessment of Quality Control (QC) metrics of the study samples and filtering of poor-quality ones, as well as an examination of the gene expression profiles between sample groups, potential detection of additional atypical samples to remove, and the exploration of the correlations between sample-level variables and their contributions in the expression variance of each gene to guide the covariate selection for DGE models. 
...
<b> 3. Differential Expression Analysis</b>: the expression of each gene is linearly modeled by the selected variables in the previous step; after fitting the model gene-wise log2-fold changes (log2FC) and *p*-values are obtained for the variable of interest and other statistics of differential expression are also computed and compared. Here DEGs are determined based on the significance threshold (controlling for the False Discovery Rate or FDR). 
...
<b> 5. DE visualization</b>: heatmaps are created to visually contrast gene expression levels of DEGs in the experimental groups. </font>
            <font size="0.8"> 
...
Evident computational steps right after sequencing involve raw sequencing reads Quality Control (QC) analysis and read alignment to a reference genome for the subsequent gene expression quantification, generating the input for DGE. Comprehensive pipelines have been developed for these purposes, such as the RNA-seq processing pipeline [*SPEAQeasy*](http://research.libd.org/SPEAQeasy/) (Eagles, N.J. *et al*. 2021) that provides a flexible, user-friendly, and reproducible pipeline to perform all suxh analyses through the implementation of a single workflow, liberating from the need of performing each step individually. 

More details of this pipeline are provided in the [original manuscript](https://doi.org/10.1186/s12859-021-04142-3), the [documentation website](http://research.libd.org/SPEAQeasy/index.html), and other in [created resources](http://research.libd.org/SPEAQeasy-example/).


### RNA-seq data processing
Once the gene expression matrix has been generated we can proceed to process the read counts. Raw counts are typically zero-enriched and not normally-distributed, which is required for the application of several statistical methods. Also gene expression . Therefore raw counts must be normalized and lowly-expressed genes filtered out.
...
<b> 2. Gene filtering</b>: distribution of $log_2(cpm+0.5)$ before and after filtering lowly-expressed genes; note the elimination of originally zero-expressed genes.
            </font>
        </figcaption>
</figure>

### Exploratory Data Analysis
The Exploratory Data Analysis (EDA) is a primordial step in which, as the name refers, we explore relevant aspects of the RNA-seq data. Particularly, the quality of the samples regarding RNA contents and read mapping rates has to be compared to identify punctual samples or groups of samples of bad quality that may have arisen by technical causes during experimental steps. Also, high biological variability can be detected and subsequently guide data partition to perform subanalyses of the data.  

Further, we are also interested in investigating trends and relationships between sample variables to hypothesize possible causes 

Sample gene expression profiles can be analyzed and compared after dimensionality reduction procedures such as Principal Component Analysis (PCA) and Multidimensional-Scaling (MDS). These analyses are useful to potentially detect samples with outlier transcriptomic profiles to further remove and to identify sample variables driving gene expression variations.

In this process we basically create tons of plots, charts and graphs to visualize the data that allow us to clearly uncover patterns and differences in the data. This allows to extract relevant information that could impact on posterior analysis, including DGE. 
...
Now, the EDA is not a well defined process that can be followed by concrete instructions or series of steps, it is not a “linear path from start to finish”. What you do, what you plot depends on the particular questions you're trying to answer, what you want to know about your data and of course, it completely depends on your own datasets. “The process is iterative, with each new discovery informing the next. And as you go, you’ll gain a deeper understanding of the data’s underlying structure and potential.” [1]

After this process (and the obvious reason why this is important) you'll have solid foundations to move forward making informed and justified decisions, test hypothesis and to draw meaningful conclusions from the analyses. At the end, all the gained insights will guide unveiling the true hidden values in the data.
...
All so-far mentioned steps are discussed and further explained in detail in the course material from last year.
