...
Once the gene expression matrix has been generated we can proceed to process the read counts. Raw counts are typically zero-enriched and not normally-distributed, which is required for the application of several statistical methods. Also raw gene expression values may reflect methodological and biological factors other than the one of interest. Therefore raw counts must be normalized and lowly-expressed genes filtered out.
...
First, the quality of the samples regarding RNA contents and read mapping rates has to be compared to (**Figure 2**: step 1):
 ...
 3) Detect high biological variability to subsequently guide data partition to perform subanalyses of the data.  
...
Other approaches include identifying outlier QC metrics (**Figure 2**: step 3), but again, caution must be taken as outlier definition is also arbitrary and could be discarding good-quality samples.  
