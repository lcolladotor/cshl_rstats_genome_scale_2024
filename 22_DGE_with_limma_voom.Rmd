...
Usually, *limma* DGE analysis is carried out in five main steps, the last four of them completed by *limma* `R` functions, as described below. We'll continue using our bulk RNA-seq data from [*smokingMouse*](https://bioconductor.org/packages/release/data/experiment/html/smokingMouse.html) to exemplify the steps. 
...
<p class="exercise">
📝 **Exercise 2**: perform a variance partition analysis (see **Chapter XX:DGE model building with variancePartition**) to select the sample variables to include in your models for DGE based on your data and create the respective design matrix. 
<p/>
...
<p class="exercise">
📝 **Exercise 3**: compute the $log(cpm)$ and the residual variance weights for each observation in your data using `voom()`.
<p/>
